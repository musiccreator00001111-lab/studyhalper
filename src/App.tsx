        ...
        <div className="w-full max-w-md h-screen md:h-[90vh] bg-slate-50 md:rounded-3xl shadow-2xl flex flex-col overflow-y-auto p-6 relative z-20 border border-slate-800/10 scrollbar-hide">
          ...
          <div className="absolute top-4 right-4 z-50">
            <button
              ...
              className="bg-white px-2.5 py-1.5 rounded-xl border border-slate-200 hover:border-slate-350 shadow-xs transition outline-none cursor-pointer text-[10px] font-black text-slate-700 flex items-center space-x-1"
              id="onboarding_lang_btn"
            >
              <span className="text-[12px]">{LANGUAGES.find(l => l.code === appLanguage)?.flag || '🌐'}</span>
              <span>{LANGUAGES.find(l => l.code === appLanguage)?.label || appLanguage}</span>
              <span className="text-[7px] text-slate-400">▼</span>
            </button>
            {showLanguageDropdown && (
              <div className="absolute right-0 top-9 w-40 bg-white border border-slate-200 rounded-2xl shadow-xl py-1.5 z-50 flex flex-col divide-y divide-slate-100 max-h-56 overflow-y-auto animate-in fade-in slide-in-from-top-2" id="onboarding_lang_dropdown">
                {LANGUAGES.map((lang) => (
                  <button
                    key={lang.code}
                    type="button"
        ...
        <div className="w-full max-w-md h-screen md:h-[90vh] bg-slate-50 md:rounded-3xl shadow-2xl flex flex-col overflow-hidden relative z-10 border border-slate-800/25">
          ...
          <div className="absolute top-4 right-4 z-50">
            <button
              type="button"
              onClick={() => setShowLanguageDropdown(!showLanguageDropdown)}
              className="bg-white/90 backdrop-blur-xs px-2.5 py-1.5 rounded-xl border border-slate-200 hover:border-slate-350 shadow-xs transition outline-none cursor-pointer text-[10px] font-black text-slate-700 flex items-center space-x-1"
              id="main_lang_btn"
            >
              <span className="text-[12px]">{LANGUAGES.find(l => l.code === appLanguage)?.flag || '🌐'}</span>
              <span>{LANGUAGES.find(l => l.code === appLanguage)?.label || appLanguage}</span>
            ...
            {showLanguageDropdown && (
              <div className="absolute right-0 top-9 w-40 bg-white border border-slate-200 rounded-2xl shadow-xl py-1.5 z-50 flex flex-col divide-y divide-slate-100 max-h-56 overflow-y-auto animate-in fade-in slide-in-from-top-2" id="main_lang_dropdown">
                {LANGUAGES.map((lang) => (
                  <button
                    key={lang.code}
                    type="button"
            ...
            >
              ...
              >
                ...
                <div className="flex justify-between items-center border-b border-dashed border-teal-600/30 pb-3 shrink-0">
                  ...
                  <span className="text-[10px] text-teal-400 font-mono font-black border border-teal-500/30 px-2 py-0.5 rounded-md bg-teal-950/50">
                    LEARN ACTIVE
                  </span>
                </div>
